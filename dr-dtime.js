...
function dtimes(d) {
	...
	if (g == 1 || g == 4 || g == 6) {
		...
		document.write((now.getDate()) + " " + months_localized['de'][now.getMonth()]);
	} else if (g == 2 || g == 5 || g == 7) {
		var now = new Date();
...
function dtime(d) {
	var now = new Date();
	now.setDate(now.getDate() + d + 1);
	document.write(days_localized['de'][now.getDay()] + " " + (now.getDate()) + ", " + " " + months_localized['de'][now.getMonth()] + " " + now.getFullYear());
}
